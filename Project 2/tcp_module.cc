    ...
    while (MinetGetNextEvent(event, timeout) == 0) {
        ...
        if ((event.eventtype == MinetEvent::Dataflow) && (event.direction == MinetEvent::IN)) {
            ...
            if (event.handle == mux) {
                ...
                cerr << list_iterator->connection << endl;
                printf("\n");
                ...
                switch(state){
                    case LISTEN:{
                        if(IS_SYN(flags)){
                            ...
                            cerr << "Listening...\n";
                            TCPState tcp_client(rand() % 500, SYN_RCVD, 5);
                            ConnectionToStateMapping<TCPState> mapping(c, Time()+2, tcp_client, true);

                            mapping.state.last_acked = mapping.state.last_sent;
                            mapping.state.SetLastRecvd(seq + 1);
                            mapping.state.last_sent = mapping.state.last_sent + 1;
                            mapping.timeout = Time() + 3; //3 second timeout
                            mapping.bTmrActive = true;
                            mapping.state.SetState(SYN_RCVD);
                            clist.push_front(mapping);


                            createPacket(mapping, 3, 0, false, snd_packet);
                    ...
                    case ESTABLISHED: {
                        ...
                        if(IS_FIN(flags)){
                            ...
                            MinetSend(mux, snd_packet);
                            Packet snd_packet2;
                            createPacket(*list_iterator, 6, 0, false, snd_packet2);
            ...
            // socket request or response has arrived
                ...
                if(list_iterator == clist.end()){
                    ...
                    switch(request.type){
                        ...
                        case CLOSE:{
                            if (state == ESTABLISHED) {
                                printf("close state established\n");
                                list_iterator->state.SetState(FIN_WAIT1);
